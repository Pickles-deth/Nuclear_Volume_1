...
def slice_has_real_nuclear_signal(sm2d, comp, guard=None):
    """
    Z端slice自身に、背景より明確な核シグナルが残っているか判定する。
    前後sliceとの連続性だけで残る偽マスクを止めるための処理。
    """
    comp=np.asarray(comp,bool)
    min_pixels=20
    if np.count_nonzero(comp)<min_pixels:
        return False, {'median_delta':0.0,'p90_delta':0.0}

    # サイドバー設定（無ければ既定値）
    min_med=float(globals().get('z_end_min_med',0.020))
    min_p90=float(globals().get('z_end_min_p90',0.035))

    inside=np.asarray(sm2d)[comp]
    near=ndi.binary_dilation(comp,np.ones((3,3),bool),iterations=5)
    if guard is not None:
        bg_region=np.asarray(guard,bool)&(~near)
    else:
        bg_region=~near
    if np.count_nonzero(bg_region)<min_pixels:
        bg_region=~comp
    outside=np.asarray(sm2d)[bg_region]
    if inside.size<min_pixels or outside.size<min_pixels:
        return False, {'median_delta':0.0,'p90_delta':0.0}

    med_delta=float(np.median(inside)-np.median(outside))
    p90_delta=float(np.percentile(inside,90)-np.percentile(outside,90))
    ok=(med_delta>=min_med and p90_delta>=min_p90)
    return bool(ok), {'median_delta':med_delta,'p90_delta':p90_delta}


def accept_z_end_slice(sm2d, comp, ref, guard=None, max_growth=4.0):
    """
    Z端の採否を、面積変化 + そのslice自身の実信号で決める。
    """
    comp=np.asarray(comp,bool); ref=np.asarray(ref,bool)
    area=int(np.count_nonzero(comp)); prev=int(np.count_nonzero(ref))
    if area<20 or prev<1:
        return False

    min_ratio=float(globals().get('z_end_min_area_ratio',0.18))
    ratio=area/float(prev)
    if ratio<min_ratio or ratio>float(max_growth):
        return False

    signal_ok,_=slice_has_real_nuclear_signal(sm2d,comp,guard)
    return bool(signal_ok)
...
def segment(full,seed,sens,bg_sigma,sigma_xy,high_scale,low_ratio,guard_px,gaps):
    ...
    for direction,start in [(-1,int(p.min())-1),(1,int(p.max())+1)]:
        ...
        while 0<=z<whole.shape[0]:
            ...
            max_growth=(3+1.5*sens)
            if np.any(comp) and accept_z_end_slice(sm[z],comp,ref,g,max_growth=max_growth):
...
def propagate_mobile_sam_mask(
    ...
    for direction in (-1, 1):
        ...
        while 0 <= z < full.shape[0]:
            ...
            if np.any(comp) and accept_z_end_slice(
                sm[z], comp, ref, guide, max_growth=(3.2 + sens)
            ):
                out[z] = comp
                ref = comp
                gap = 0
...
st.sidebar.subheader('Z端の終了判定'); z_end_min_area_ratio=st.sidebar.slider('Z端 最小面積比',0.05,0.50,0.18,0.01,help='前sliceの面積に対してこれ未満まで急減した候補は核の終端として除外します。'); z_end_min_med=st.sidebar.slider('Z端 中央値シグナル差',0.000,0.100,0.020,0.005,help='候補内の中央値が背景よりこの値以上高い場合だけ採用します。'); z_end_min_p90=st.sidebar.slider('Z端 強シグナル差',0.000,0.150,0.035,0.005,help='候補内90パーセンタイルが背景よりこの値以上高い場合だけ採用します。')

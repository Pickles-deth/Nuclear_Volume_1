...
def detect_nucleus_candidates(
    ...
    for i, comp in enumerate(components, start=1):
        ...
        # 【修正点1】Zスタック端の切り落としを防止するため、Z方向は常に全範囲を選択
        z0 = 0
        z1 = zdim
...
def _make_nucleus_xy_guard(
    ...
    # 【修正点2】MIPの二値化閾値を引き下げ、低輝度な外形領域もガード内に含める
    thr2d = float(np.clip(otsu2d * 0.30, 0.005, 0.95))
...
def _best_overlapping_component_2d(mask2d, reference2d, dilation_px=3):
    ...
    min_overlap = max(1, int(round(np.count_nonzero(reference2d) * 0.005)))
...
def expand_seed_mask_full_z(
    ...
    """
    Full-Z whole-nucleus segmentation.
    """
    ...
    high_thr = float(np.clip(otsu * float(threshold_factor), 0.01, 0.95))

    weak_ratio = float(np.clip(0.60 - 0.40 * sensitivity, 0.15, 0.60))
    low_thr = float(np.clip(high_thr * weak_ratio, 0.005, high_thr))
    ...
    z_tail_thr = float(np.clip(low_thr * 0.50, 0.002, low_thr))
    overlap_dilation = 4 + int(round(4 * sensitivity))
    ...
    for direction, first_z in [(-1, start_z - 1), (1, end_z + 1)]:
        ...
        while 0 <= z < whole.shape[0]:
            ...
            if np.any(comp):
                ...
                max_growth = 4.0 + 1.0 * sensitivity
                if area <= max(prev_area * max_growth, prev_area + 500):
                    whole[z] = comp
                    reference = comp
                    tracked += 1
                    gap = 0
...
def analyze_candidates(
    ...
    for idx, c0 in enumerate(candidates, start=1):
        ...
        # 【修正点3】Z端が暗く途切れやすい画像向けに max_gap_slices を 5 に拡大
